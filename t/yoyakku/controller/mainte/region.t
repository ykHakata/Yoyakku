...
subtest 'method' => sub {
    my $obj = Yoyakku::Controller::Mainte::Region->new();
    isa_ok( $obj, 'Yoyakku::Controller::Mainte::Region' );

    my @methods = qw{index mainte_region_serch};
...
subtest 'mainte_region_serch' => sub {
    $t->post_ok( '/root_login' => form => $login_params );
    my $msg = qr{\Q地域区分マスター／テーブル[region]\E};
    $t->get_ok('/mainte_region_serch')->status_is(200)->content_like($msg);
    $t->get_ok('/up_logout');
};

=head2 mainte_region_new

    region テーブルに新規レコード追加、既存レコード修正

=cut

subtest 'mainte_region_new' => sub {
